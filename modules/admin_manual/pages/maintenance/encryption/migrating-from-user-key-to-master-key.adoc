...
After re-enabling the encryption, it is possible that some or all users see a re-synchronisation of their data from the server to the desktop client, especially for shared folders. An information to the users (e.g. through the announcement app) prior to the migration process is recommended.
